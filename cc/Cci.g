 ...
 *
 * $Id: Cci.g,v 1.1 2003-05-05 10:29:09 cvsg Exp $
 */
...
header { package caesar.ci.compiler; }
{
  import java.util.ArrayList;

  import familyj.compiler.*;
  
  import caesar.ci.compiler.ast.*;
...
class CciParser extends Parser;
options {
...
{
  public CciParser(Compiler compiler, InputBuffer buffer, KjcEnvironment environment) {
    super(compiler, new CciScanner(compiler, buffer), MAX_LOOKAHEAD);
...
:
  (
    ...
    { 
    	self = new JPackageName(buildTokenReference(), 
      	                        name, 
      	                        getStatementComment()); 
    }
  )?
...
:
{
  context.addPackageImport(new JPackageImport(
    TokenReference.NO_REF, "java/lang", null));
}
;
...
:
  ...
  (
    DOT j:IDENT
      {
      	(buffer == null 
      	  ? (buffer = new StringBuffer(i.getText())) 
      	  : buffer).append('/').append(j.getText()); 
      }
  )*
  {
  	name = buffer == null 
  ...
  SEMI
  {
  	if (star) 
  		context.addPackageImport(new JPackageImport(
  		  buildTokenReference(), name, getStatementComment()));
    else 
    	context.addClassImport(new JClassImport(
    	  buildTokenReference(), name, getStatementComment()));
  }
;
...
// andreas start
|
  "virtual" { self = at.dms.kjc.Constants.FJC_VIRTUAL; }
|
  "override" { self = at.dms.kjc.Constants.FJC_OVERRIDE; }
|
  "clean" { self = at.dms.kjc.Constants.FJC_CLEAN; }
// andreas end
// Walter start
|
  "collaboration" { self = at.dms.kjc.Constants.CCI_COLLABORATION; }
|
  "provided" { self = at.dms.kjc.Constants.CCI_PROVIDED; }
|
  "expected" { self = at.dms.kjc.Constants.CCI_EXPECTED; }
// Walter end
;
...
:
  ...
  jClassBlock[context]
  {
      ...
      if (environment.getAssertExtension() == KjcEnvironment.AS_ALL) 
      {
          ...
          methods = new JMethodDeclaration[
                          assertions.length + decMethods.length];
          // assertions first!
          System.arraycopy(assertions, 0, methods, 
                           0, assertions.length);
          System.arraycopy(decMethods, 0, methods, 
                           assertions.length, decMethods.length);
      }
      else
        methods = context.getMethods();
        
      if (bindings != CReferenceType.EMPTY 
          || interfaces != CReferenceType.EMPTY)
      {
         self = new CciClassDeclaration(sourceRef,
                   modifiers,
                   ident.getText(),
                   typeVariables,
                   superClass,
               ...
               at.dms.kjc.Constants.FJC_OVERRIDE)) 
      {
          self = new FjOverrideClassDeclaration(sourceRef,
               ...
               at.dms.kjc.Constants.FJC_VIRTUAL))
      {
          self = new FjVirtualClassDeclaration(sourceRef,
               ...
               at.dms.kjc.Constants.FJC_CLEAN))
      {
...
:
  ...
  jClassBlock[context]
  {
     if (CModifier.contains(modifiers, 
     		at.dms.kjc.Constants.CCI_COLLABORATION)) 
     {
     ...
     {
        self = new FjInterfaceDeclaration(sourceRef,
                                           modifiers,
                                           ident.getText(),
                                           typeVariables,
                                           interfaces,
...
jConstructorDefinition [CParseClassContext context, int modifiers]
      ...
      if (environment.getAssertExtension() == KjcEnvironment.AS_ALL) {
        ...
        self = new KopiConstructorDeclaration(sourceRef,
                                              ...
                                              new KopiConstructorBlock(sourceRef,
                                                                       constructorCall,
                                                                       (JStatement[]) body.toArray(new JStatement[body.size()])), //at.dms.util.Utils.toArray(body, JStatement.class)),
                                              javadoc,
                                              comments,
                                              pre,
                                              post,
...
jMethodDefinition [CParseClassContext context, int modifiers, CType type, CTypeVariable[] typeVariables]
...
:
  ...
  { self = (JExpression[])vect.toArray(new JExpression[vect.size()]); } //at.dms.util.Utils.toArray(vect, JExpression.class); }
;
...
:
  ...
  { self = (JExpression[]) container.toArray(new JExpression[container.size()]); } //at.dms.util.Utils.toArray(container, JExpression.class); }
;
