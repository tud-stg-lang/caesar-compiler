 ...
 *
 * $Id: JOwnerExpression.java,v 1.1 2003-07-05 18:29:38 werner Exp $
 */
  ...
  public JExpression analyse(final CExpressionContext context) throws PositionedError {
	...
	if (expr == null) {
	  if (context.getMethodContext() instanceof CConstructorContext) {
	    ...
	    expr = new FjFieldAccessExpression(getTokenReference(), new JThisExpression(getTokenReference()), JAV_OUTER_THIS);
	    // andreas end
	  }
	} else {
	  // andreas start
	  //expr = new JFieldAccessExpression(getTokenReference(), expr, JAV_OUTER_THIS);
	  expr = new FjFieldAccessExpression(getTokenReference(), expr, JAV_OUTER_THIS);
      // andreas end
	}
