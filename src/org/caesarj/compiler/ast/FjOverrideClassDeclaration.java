...
public class FjOverrideClassDeclaration extends FjVirtualClassDeclaration implements FjOverrideable  {
	...
	protected FjCleanClassInterfaceDeclaration newInterfaceDeclaration(
		...
		return new FjOverridingCleanClassInterfaceDeclaration(where, ident, interfaces, methods, getOwnerDeclaration(), this );
	}
	protected FjCleanClassIfcImplDeclaration newIfcImplDeclaration(
		...
		CReferenceType[] interfaces,
		FjCleanMethodDeclaration[] methods ) {
		return new FjOverridingCleanClassIfcImplDeclaration(where, ident, interfaces, methods, getOwnerDeclaration(), this );
	}
	...
	public static void setOverridingSuperClass( FjOverrideable instance, Compiler compiler ) {
		...
		while( superClass == null
			&& !fjts.superClassOf( ownerSuper ).getQualifiedName().equals( "java/lang/Object" )
			&& !fjts.superClassOf( ownerSuper ).getQualifiedName().equals( FjConstants.CHILD_TYPE_NAME )
			&& !fjts.superClassOf( ownerSuper ).getQualifiedName().equals( FjConstants.CHILD_IMPL_TYPE_NAME ) ) {
			...
			if( FjConstants.isIfcImplName( fjts.superClassOf( ownerSuper ).getQualifiedName() ) )
				ownerSuper = fjts.superClassOf( ownerSuper ).getInterfaces()[0].getCClass();
			else
			...
			for( int i = 0; innerSuper == null && i < inners.length; i++ ) {
				if( inners[ i ].getIdent().equals(
					FjConstants.toIfcName( instance.getCClass().getIdent() ) ) )
					innerSuper = inners[ i ].getCClass();
			}
			if( innerSuper != null ) {
				superClass = new CClassNameType(
	...
	public Vector inherritConstructorsFromBaseClass(Hashtable markedVirtualClasses)
		...
		if( getSuperClass().getQualifiedName().equals( FjConstants.CHILD_IMPL_TYPE_NAME ) )
			return messages;		
		...
		try {
			...
			String superClassKey = FjConstants.toFullQualifiedBaseName(
				getSuperClass().getQualifiedName(), self );
			FjCleanClassDeclaration classDecl = 
				(FjCleanClassDeclaration) markedVirtualClasses.get ( superClassKey );
			if( classDecl != null )
			...
			if( constructorsToSupport == null ) {
				CReferenceType superBaseType =
					new CClassNameType( FjConstants.toFullQualifiedBaseName(
						getSuperClass().getIdent(), self ) );
				superBaseType = (CReferenceType) superBaseType.checkType( self );
				constructorsToSupport = new CheckedConstructors( superBaseType.getCClass() );
			}
